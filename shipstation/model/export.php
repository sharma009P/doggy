...
class ModelExport extends Model {
    ...
    public function getOrderData($order_data) {
        ...
        if (!empty($order_data)) {
            ...
            /* Custom SKU */
            $order_data['OrderNumber'] = $order_data['order_id'];
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_data['OrderNumber'] . "'");
            ...
            $order_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_data['OrderNumber'] . "' AND order_product_id = '" . (int)$order_product_id . "'");
                
            $product_stocks = array();

    		$product_query = $this->db->query("SELECT s.*, pov.product_option_id, so.product_option_value_id "
    		...
    		foreach ($product_query->rows as $row) {
    			if ($row['combination_id'] != $combination_id) {
    				...
    				$product_stocks[] = $combination;
    			}
                
    			$option_values[$row['product_option_id']] = $row['product_option_value_id'];
    		}
                   
            foreach ($order_query->rows as $option) {
                $new_options_value[$option['product_option_id']] = $option['product_option_value_id'];
            }
            $product_n_sku = '';
            foreach($product_stocks as $product_stock){
                $_array_compare_result = array_intersect_assoc($product_stock['product_option_values'], $new_options_value);
                if(!empty($_array_compare_result)){
                    $product_n_sku = $product_stock['sku'];
                }
            }
            /* //Custom SKU */
            ...
            foreach ($order_products as $product) {
                ...
                $products[] = array(
                    ...
                    'SKU' =>  ($product_n_sku != '')?$product_n_sku:(($product['sku'] == '') ? (($product['upc'] == '') ? $product['model'] : $product['upc']) : $product['sku']),
