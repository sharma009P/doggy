...
class ModelCheckoutOrder extends Model {
	...
	public function update_shipping($order_id,$data){
	
	    $this->db->query("UPDATE `" . DB_PREFIX . "order` SET shipping_firstname = '" . $this->db->escape($data['full_name']) . "', shipping_address_1 = '" . $this->db->escape($data['address_1']) . "', shipping_address_2 = '" . $this->db->escape($data['address_2']) . "', shipping_country = '" . $this->db->escape($data['country']) . "', shipping_country_id = '" . (int)$data['country_id'] . "', shipping_city = '" . $this->db->escape($data['city']) . "', shipping_postcode = '" . $this->db->escape($data['postcode']) . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
  // email = '" . $this->db->escape($data['email']) . "'
  return $order_id;
	}

	public function update_billing($order_id,$data){
	    $this->db->query("UPDATE `" . DB_PREFIX . "order` SET payment_firstname = '" . $this->db->escape($data['full_name']) . "', payment_address_1 = '" . $this->db->escape($data['address_1']) . "', payment_address_2 = '" . $this->db->escape($data['address_2']) . "', payment_country = '" . $this->db->escape($data['country']) . "', payment_country_id = '" . (int)$data['country_id'] . "', payment_city = '" . $this->db->escape($data['city']) . "', payment_postcode = '" . $this->db->escape($data['postcode']) . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
  // email = '" . $this->db->escape($data['email']) . "'
  return $order_id;
	}
		public function update_shipping_email($order_id,$data){
	
	    $this->db->query("UPDATE `" . DB_PREFIX . "order` SET email = '" . $this->db->escape($data['email']) . "', date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
    	...
    	if ( StockUtils::isStockModuleEnabled($this->config) ) {
    	  ...
    	  if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
					...
					$stock_model->chargeOrderOptionsStock($order_id, $sm_product_query->rows, false);
    	  }
				if (in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && !in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {    	
					//$this->log->write('Reclaiming stock');
					$sm_product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");
					$stock_model->chargeOrderOptionsStock($order_id, $sm_product_query->rows, true);
				}
    	} 
