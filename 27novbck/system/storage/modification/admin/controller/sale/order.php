		...
		if ($order_info) {
            ...
            foreach ($products as $product) {
                ...
                $query = $this->db->query("SELECT s.*, pov.product_option_id, so.product_option_value_id "
        				...
        				. " WHERE s.product_id = '" . (int)$product['product_id'] . "' ORDER BY s.combination_id, pov.product_option_id, so.product_option_value_id ");
				...
				foreach ($options as $option) {
                    ...
                    if ($option['type'] != 'file') {
                        ...
                        $sku_details = $this->db->query("SELECT sku FROM ".DB_PREFIX."product_option_value WHERE product_option_id = ".$option['product_option_id']." AND product_option_value_id = ".$option['product_option_value_id'])->row;
                        $option_data[] = array(
							'name'  => $option['name'],
							'value' => $option['value'],
							'type'  => $option['type'],
                ...
                foreach($product_stocks as $product_stock){
                    $count = count(array_intersect_assoc($product_stock['product_option_values'],$new_options_value)).'<br/>';
                    if($count >= '2'){
