...
class ControllerCatalogProduct extends Controller {
	...
	public function index() {
       ...
       $query = $this->db->query("DESC ".DB_PREFIX."product_option_value sku");
        ...
        if (!$query->num_rows) {
            ...
            $this->db->query("ALTER TABLE `oc_product_option_value` ADD `sku` VARCHAR(64) NOT NULL AFTER `product_id`");
  	...
  	$this->load->language(StockModuleConstants::$STOCK_LANGUAGE_FILE);
	...
	public function add() {
  	...
  	$this->load->language(StockModuleConstants::$STOCK_LANGUAGE_FILE);
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$return_id = $this->model_catalog_product->addProduct($post_array);
            
            for($i=1;$i<=2;$i++){
                if(isset($this->request->files['_rec_file_upload_'.$i]['name'])){
                        
                    $temp_file = $_FILES['_rec_file_upload_'.$i]['tmp_name'];
                    $file_name = $_FILES['_rec_file_upload_'.$i]['name'];
                    
                    $recommended_product_folder = DIR_IMAGE.'recommended_product/';
                
                    if(!is_dir($recommended_product_folder)) {
                        mkdir($recommended_product_folder, 0777, true);
                    }
                    
                    $target_path = $recommended_product_folder.$return_id.'/';
                    ...
                    if(move_uploaded_file($temp_file, $target_file)){
                        $this->request->post['rec_file_upload_'.$i] = $file_name;
                    }
                }
                $this->db->query("UPDATE ".DB_PREFIX."product SET `rec_file_upload_".$i."` = '".$file_name."' WHERE `product_id` = " .(int)$return_id);
	...
	public function edit() {
  	...
  	$this->load->language(StockModuleConstants::$STOCK_LANGUAGE_FILE);
        ...
        $this->request->post['rec_file_upload_1'] = ($this->request->post['hide_rec_file_upload_1'])?$this->request->post['hide_rec_file_upload_1']:'';
        $this->request->post['rec_file_upload_2'] = ($this->request->post['hide_rec_file_upload_2'])?$this->request->post['hide_rec_file_upload_2']:'';
        ...
        for($i=1;$i<=2;$i++){
            if(isset($this->request->files['_rec_file_upload_'.$i]['name'])){
                ...
                $target_path = $recommended_product_folder.$product_id.'/';
        ...
        if($this->config->get('module_marketplace_status')) {
			...
			$return = $this->model_catalog_product->editProduct($this->request->get['product_id'], $this->request->post);
    ...
    $stock_model = StockUtils::loadModel($this, StockModuleConstants::$STOCK_MODEL);
  	...
  	$this->load->language(StockModuleConstants::$STOCK_LANGUAGE_FILE);
        ...
        $stock_model->deleteStock($product_id);
			}
    ...
    $data['entry_no_all_options'] = $this->language->get('sm_entry_no_all_options'); 
        ...
        $data['c_product_id'] = $this->request->get['product_id'];
        ...
        if (isset($this->request->post['cart_recommended'])) {
			$data['cart_recommended'] = $this->request->post['cart_recommended'];
		} elseif (!empty($product_info)) {
        ...
        if (isset($this->request->files['rec_file_upload_1']['name'])) {
            $data['rec_file_upload_1'] = $this->request->files['rec_file_upload_1']['name'];
		} elseif (!empty($product_info)) {
			$data['rec_file_upload_1'] = $product_info['rec_file_upload_1'];
		} else {
			$data['rec_file_upload_1'] = '';
		}
        
        if (isset($this->request->files['rec_file_upload_2']['name'])) {
			$data['rec_file_upload_2'] = $this->request->files['rec_file_upload_2']['name'];
		} elseif (!empty($product_info)) {
			$data['rec_file_upload_2'] = $product_info['rec_file_upload_2'];
		} else {
			$data['rec_file_upload_2'] = '';
		}
        
        if (isset($this->request->post['front_company_name'])) {
			$data['front_company_name'] = $this->request->post['front_company_name'];
		} elseif (!empty($product_info)) {
			$data['front_company_name'] = $product_info['front_company_name'];
		} else {
			$data['front_company_name'] = '';
		}
		...
		if (isset($this->request->post['size_guide'])) {
			$data['size_guide'] = $this->request->post['size_guide'];
		} elseif (!empty($product_info)) {
			$data['size_guide'] = $product_info['size_guide'];
		} else {
			$data['size_guide'] = 1;
		}
		if (isset($this->request->post['qty_p_enable'])) {
			$data['qty_p_enable'] = $this->request->post['size_guide'];
		} elseif (!empty($product_info)) {
        ...
        $partner_details = $this->model_catalog_product->getFrontCompanyNames($product_info['product_id']);
        
        $company_names = array();
        if(!empty($partner_details['companynames'])){
            $company_names = explode(',',$partner_details['companynames']);
        }
        
        $data['front_company_names'] = $company_names;
    ...
    if (isset($this->request->post['allow_preorder'])) {
		...
		if (isset($this->request->post['shipping_day_avail'])) {
			$data['shipping_day_avail'] = $this->request->post['shipping_day_avail'];
		} elseif (!empty($product_info)) {
			$data['shipping_day_avail'] = $product_info['shipping_day_avail'];
		} else {
			$data['shipping_day_avail'] = 0;
		}
		...
		foreach ($product_options as $product_option) {
			...
			$data['product_options'][] = array(
                ...
                'price'                => isset($product_option['price']) ? $product_option['price'] : '',
...
$data['stock_module_enabled']= StockUtils::isStockModuleEnabled($this->config);
    ...
    if (StockUtils::isStockModuleEnabled($this->config)) {
      if (isset($this->request->post['product_option'])) {
        $info = StockUtils::getOptionsInfo($this->request->post['product_option']);
        if ($info['error'] > 0) {
          if ($info['error'] === 1) {
