...
'allow_preorder'             => $query->row['allow_preorder'],
				...
				'size_guide'      => $query->row['size_guide'],
				'qty_p_enable'      => $query->row['qty_p_enable'],
				'shipping_day_avail'      => $query->row['shipping_day_avail'],
                ...
                'free_shipping'    => $query->row['free_shipping'],
                'cart_recommended' => $query->row['cart_recommended'],
                'rec_file_upload_1' => $query->row['rec_file_upload_1'],
                'rec_file_upload_2' => $query->row['rec_file_upload_2'],
                'front_company_name'=> $query->row['front_company_name'],
				'upc'              => $query->row['upc'],
				'ean'              => $query->row['ean'],
				'jan'              => $query->row['jan'],
				'isbn'             => $query->row['isbn'],
    ...
    public function getCartRecommended($limit) {
	   	$product_data = array();

		$query = $this->db->query("SELECT * FROM `".DB_PREFIX."product` WHERE status = '1' AND date_available <= NOW() AND cart_recommended = 1 LIMIT ". (int)$limit);
		...
		foreach ($query->rows as $result) {
            ...
            $product_data[$result['product_id']]['options'] = $this->getProductOptions($result['product_id']);
		}
    ...
    public function getFrontCompanyName(){
