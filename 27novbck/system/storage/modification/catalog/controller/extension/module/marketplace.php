...
class ControllerExtensionModuleMarketplace extends Controller {
	...
	public function index() {
		...
		$this->load->language('extension/module/marketplace');
                   ...
                   //follow seller
                    $data['text_follow_seller']     =   $this->language->get('text_follow_seller');
                    $data['text_followseller']      =   $this->language->get('text_follow_seller');
                    $data['text_unfollow_seller']   =   $this->language->get('text_unfollow_seller');
		...
		if(isset($this->request->get['route']) AND (substr($this->request->get['route'],0,8)=='account/')) {
	    	 ...
	    	 */
                   ...
                   */
                   if ($this->config->get('module_wk_mpfollowseller_status') && $this->config-> get('wk_mpfollowseller_seller_dashboard_status') ) {
                       $data['account_menu_href']['followseller'] = $this->url->link('account/customerpartner/wk_mpfollowseller', '', 'SSL');
                  } else {
                      $data['module_wk_mpfollowseller_status'] = false;
                      if(isset($data['account_menu_href']['followseller'])) {
                          unset($data['account_menu_href']['followseller']);
                      }
			...
			if(isset($id['id']) AND $id['id']){
				...
				if($partner){
                  ...
                  if($this->config->get('module_wk_mpfollowseller_status')){
                    ...
                    if (isset($partner) && isset($this->session->data['customer_id'])) {
                       /**
                       * [$checkCustomerFollow check whether this customer follow this seller or not ]
                       * @var [array]
                       */
                      $checkCustomerFollow = $this->model_extension_module_wk_mpfollowseller->checkCustomerFollow($this->session->data['customer_id'],$partner['customer_id']);

                      if (!empty($checkCustomerFollow)) {
                          $customerfollows = 1;
                      }else{
                           $customerfollows = 0;
                      }
